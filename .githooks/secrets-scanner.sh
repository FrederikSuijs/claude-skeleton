...
#   SCAN_MODE          - "warn" (log only) or "block" (exit non-zero on findings) (default: block)
#   SCAN_SCOPE         - "diff" (changed files only) or "staged" (staged files) (default: staged)
...
echo "🔍 Scanning ${#FILES[@]} staged file(s) for secrets..."
...
if [[ $FINDING_COUNT -gt 0 ]]; then
  echo ""
  echo "⚠️  Found $FINDING_COUNT potential secret(s) in staged files:"
  echo ""
  ...
  if [[ "$MODE" == "block" ]]; then
    echo "🚫 Commit blocked: resolve the findings above before committing."
    echo "   Set SKIP_SECRETS_SCAN=true to bypass (e.g. SKIP_SECRETS_SCAN=true git commit ...),"
    echo "   or add patterns to SECRETS_ALLOWLIST."
    exit 1
  else
